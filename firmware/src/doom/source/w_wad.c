...
#define BOOT_LOG_LINES 12
static char boot_log[BOOT_LOG_LINES][48];
static int boot_log_count = 0;

static void wad_boot_stage(const char *stage)
{
    printf("Boot stage: %s\r\n", stage);

    if (boot_log_count < BOOT_LOG_LINES) {
        strncpy(boot_log[boot_log_count], stage, 47);
        boot_log[boot_log_count][47] = '\0';
        boot_log_count++;
    } else {
        for (int i = 0; i < BOOT_LOG_LINES - 1; i++) {
            strcpy(boot_log[i], boot_log[i + 1]);
        }
        strncpy(boot_log[BOOT_LOG_LINES - 1], stage, 47);
        boot_log[BOOT_LOG_LINES - 1][47] = '\0';
    }

    lcd_fill_rect(8, 60, 304, 172, COLOR_BLACK);
    for (int i = 0; i < boot_log_count; i++) {
        font_draw_string(12, 64 + i * 14, boot_log[i], COLOR_WHITE, COLOR_BLACK, &font_small);
    }
}
...
volatile doom_flash_diag_t doom_flash_diag __attribute__((section(".flash_diag")));
...
extern uint8_t doom_framebuffer[];
uint32_t currentSpiAddress = 0;
...
void initImmutableFlashRegion()
{
    ...
    _g->screens[0].data = (uint8_t*) doom_framebuffer;
...
#endif
    wad_boot_stage("FLASH INIT DONE");
    //
    #if CACHE_ALL_COLORMAP_TO_RAM
...
{
    // Not used in AT32 DOOM port
}

void flashErasePage(uint32_t *pageAddress)
{
    flash_unlock();
    flash_flag_clear(FLASH_BANK1_PRGMERR_FLAG | FLASH_BANK1_EPPERR_FLAG | FLASH_BANK1_ODF_FLAG);
    flash_flag_clear(FLASH_BANK2_PRGMERR_FLAG | FLASH_BANK2_EPPERR_FLAG | FLASH_BANK2_ODF_FLAG);
    flash_status_type status = flash_sector_erase((uint32_t)pageAddress);
    flash_lock();
    if (status != FLASH_OPERATE_DONE) {
        char err_msg[64];
        snprintf(err_msg, sizeof(err_msg), "Erase fail: %d", status);
        I_Error(err_msg);
    }
}
...
{
    /* Erased flash already contains all ones. Programming 0xFFFFFFFF is both
     * unnecessary and rejected by this controller during page restoration. */
    if (word == 0xFFFFFFFFU) {
        return;
    }
    ...
    if (*address == 0xFFFFFFFF)
    {
        ...
        flash_flag_clear(FLASH_BANK1_PRGMERR_FLAG | FLASH_BANK1_EPPERR_FLAG | FLASH_BANK1_ODF_FLAG);
        flash_flag_clear(FLASH_BANK2_PRGMERR_FLAG | FLASH_BANK2_EPPERR_FLAG | FLASH_BANK2_ODF_FLAG);
        flash_status_type status = flash_word_program((uint32_t)address, word);
        __DSB(); /* drain store buffer so readback goes to actual flash cell */
        __ISB(); /* flush instruction pipeline after flash op */
        flash_lock();

        uint32_t val_after = *(volatile uint32_t *)address;

        if (status != FLASH_OPERATE_DONE || val_after != word) {
            I_Error("Flash prog failed addr=%08lX st=%d", (uint32_t)address, (int)status);
        }
    }
}
/**
...
void cacheLumpNamesToFlash(void)
{
  ...
  //
  wad_boot_stage("NAMES ALLOC");
  lumpNames = Z_Calloc(8, _g->numlumps, PU_STATIC, NULL);
  wad_boot_stage("NAMES READ");
  //
