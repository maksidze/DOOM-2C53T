...
static inline void fastOpeningsCopy(short *dest, const short *source, int num);
...
// Both pointers may only be halfword-aligned. Word copies can make GCC emit
// LDRD/STRD, which fault when UNALIGN_TRP is enabled on Cortex-M4.
static inline void fastOpeningsCopy(short *dest, const short *source, int num)
{
    for (int i = 0; i < num; i++) {
        dest[i] = source[i];
...
// R_RenderView
//
#define UPDATE_NET_DURING_RENDERING 0
