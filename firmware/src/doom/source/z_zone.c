...
#define MAX_STATIC_ZONE (80 * 1024)
...
static bool zone_block_valid(const memblock_t *block)
{
    uintptr_t address = (uintptr_t)block;
    uintptr_t start = (uintptr_t)staticZone;
    uintptr_t end = start + sizeof(staticZone);
    return address >= start && address + sizeof(*block) <= end &&
           (address & (CHUNK_SIZE - 1U)) == 0;
}

static void zone_corruption_fault(memblock_t *base, uint16_t next_sptr,
                                  uint32_t request_size)
{
    char detail[64];
    snprintf(detail, sizeof(detail), "B:%08lX N:%04X S:%lu",
             (uint32_t)base, next_sptr, request_size);
    taskDISABLE_INTERRUPTS();
    fault_display("ZONE BAD LINK", detail);
    for (;;) {}
}
//
// Z_Malloc
...
void* Z_Malloc2(uint32_t size, int tag, void **user, const char *sz)
{
  ...
  (void) sz;
    ...
    if (extra > MINFRAGMENT)
    {
        ...
        memblock_t *nextblock = getMemblockNext(newblock);
        if (!zone_block_valid(nextblock)) {
            zone_corruption_fault(base, newblock->next_sptr, size);
        }
        nextblock->prev_sptr = getShortPtr(newblock);
