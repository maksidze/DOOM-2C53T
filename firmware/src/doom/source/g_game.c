...
#define MAX_HOST_NAME_LENGTH 15
#define START_MAP 1
...
void G_UpdateSaveGameStrings()
{
    ...
    byte *loadbuffer = (byte*) doom_framebuffer;
...
static void G_DoSaveGame(boolean menu)
{
    ...
    // Therefore we can "safely" use it as a temporary work buffer.
    // Yes, this is a bad hack, but it works :)
    byte *savebuffer = (byte*) doom_framebuffer;
    // read old data
    extMemSetCurrentAddress(SAVEGAME_ADDRESS);
...
void G_LoadSettings()
{
    doom_save_settings_t *settings = (doom_save_settings_t*) doom_framebuffer;
